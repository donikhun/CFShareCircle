...
#define kFacebookAppID @"198142280345578"
#define kPinterestClientId @"1435709"
...
- (void)shareToMail:(Block)completionBlock {
  if ([MFMailComposeViewController canSendMail]) {
    ...
    if (self.shareView.parentViewController.presentedViewController) {
...
- (void)shareToFacebook:(Block)completionBlock {
  if (![SLComposeViewController composeViewControllerForServiceType:SLServiceTypeFacebook]) {
    [SIAlertView presentErrorWithMessage:@"Could not access Facebook"];
    return;
  }
  
  SLComposeViewController *facebookComposeView = [SLComposeViewController composeViewControllerForServiceType:SLServiceTypeFacebook];
  [facebookComposeView setInitialText:@"via #societyoflacquer"];
  [facebookComposeView addImage:self.shareView.params[@"image"]];
  
  [facebookComposeView setCompletionHandler:^(SLComposeViewControllerResult result) {
    completionBlock();
  }];
  
  if (self.shareView.parentViewController.presentedViewController) {
    [self.shareView.parentViewController dismissViewControllerAnimated:YES completion:^{
      [self.shareView.parentViewController presentViewController:facebookComposeView animated:YES completion:nil];
    }];
  }
  else {
    [self.shareView.parentViewController presentViewController:facebookComposeView animated:YES completion:nil];
...
- (void)shareToPinterest:(Block)completionBlock {
  ...
  if ([_pinterest canPinWithSDK]) {
    ...
    [SIAlertView presentErrorWithMessage:@"Please install the Pinterest app!"];
  }
  completionBlock();
}
...
- (void)shareToTwitter:(Block)completionBlock {
  ...
  SLComposeViewController *twitterComposeView = [SLComposeViewController composeViewControllerForServiceType:SLServiceTypeTwitter];
  [twitterComposeView setInitialText:@"via #societyoflacquer"];
  [twitterComposeView addImage:self.shareView.params[@"image"]];
  
  [twitterComposeView setCompletionHandler:^(SLComposeViewControllerResult result) {
    completionBlock();
  }];
  
  if (self.shareView.parentViewController.presentedViewController) {
    [self.shareView.parentViewController dismissViewControllerAnimated:YES completion:^{
      [self.shareView.parentViewController presentViewController:twitterComposeView animated:YES completion:nil];
    }];
  }
  else {
    [self.shareView.parentViewController presentViewController:twitterComposeView animated:YES completion:nil];
