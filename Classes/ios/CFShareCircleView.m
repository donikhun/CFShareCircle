...
- (void)createSharerLinks {
  for (CFSharer *sharer in _sharers) {
    sharer.shareView = self;
  }
}
...
- (void)setUpCircleLayers {
  ...
  for(int i = 0; i < [self numberOfSharersInCircle]; i++) {
    ...
    if(i == 5 && _sharers.count > 6)
      sharer = [[CFSharer alloc] initWithName:@"More" imageName:@"more.png" type:CFSharerTypeMore];
    else
...
- (void)hideCircle {
  ...
  [_delegate shareCircleDidCancel];
}
